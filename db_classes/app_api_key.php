...
class App_API_Key extends Data_Access {
	...
	public function getRecordByAPIKey($varAPIKey = NULL) {
		...
		$this::validateRequest($varAPIKey);
	...
	public function getUsers($user_id = null){
		...
		$this::validateRequest($user_id);
		$query = "SELECT * FROM " . CONST_DB_SCHEMA . ".users";
		if($user_id)
		$query .= " WHERE (user_id = '" . $user_id . "')";
		$res = $this->getResultSetArray($query);
		// if nothing comes back, then return a failure

		if ($res['response'] !== '200' && $res['response'] !== '204') {
			$responseArray = App_Response::getResponse('403');
		} else {
			$responseArray = $res;
		}
	...
	static function validateRequest($varAPIKey = null) {
